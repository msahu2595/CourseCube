...
import {CREATE_ADVERT, EDIT_ADVERT} from '@mutations';
...
const AdminAdvertListScreen = () => {
  ...
  return (
    <SafeAreaContainer>
      ...
      />
      <EditAdvertModal
        visible={editAdvertModal}
        onClose={() => setEditAdvertModal(null)}
      />
    </SafeAreaContainer>
  );
};
...
const EditAdvertModal = ({visible, onClose}) => {
  const [editAdvert, {loading: mutationLoading}] = useMutation(EDIT_ADVERT, {
    onCompleted: data => {
      console.log('onCompleted', data);
      onClose();
      showMessage({
